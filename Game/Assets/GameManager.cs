...
public class GameManager : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		GUI.TextField(new Rect(0,0,width,height), ""+numKills, counterStyle);
			if(players.Length > 1)
			{
				...
				GUI.TextField(new Rect(Screen.width-width, 0, width, height), ""+numKills, counterStyle);
				if(players.Length > 2)
				{
					...
					GUI.TextField(new Rect(0, Screen.height-height, width, height), ""+numKills, counterStyle);
					if(players.Length > 3)
					{
						...
						GUI.TextField(new Rect(Screen.width-width, Screen.height-height, width, height), ""+numKills, counterStyle);
					}
				}
			}
		}
